...
class InvoiceProcessor:
    ...
    def save(self, path: str) -> None:
        ...
        out_path = downloads / file_name
        out.to_excel(out_path, index=False)
        logger.info(f"Счёт сохранён в {out_path}")
...
class App:
    ...
    def build_invoice(self) -> None:
        ...
        messagebox.showinfo("Готово", f"Новый счёт сохранён: {Path.home() / 'Downloads' / out_path}")
...
if __name__ == "__main__":
    ...
    if args.cli:
        ...
        print(f"\nФайл сохранён: {Path.home() / 'Downloads' / out_path}")
    else:
        app = App()
        if args.rules:
            try:
